    ...
    public partial class Settle : Form
    {
        ...
        private void btnEnter_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Недостатня сума, будь ласка, введіть правильну суму!", "Попередження", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                else
                {
                    ...
                    {
                        db.openConnection();
                        cm = new SqlCommand("UPDATE AutoParts SET QuantityInStock = QuantityInStock - " + int.Parse(cashier.dgvCash.Rows[i].Cells[5].Value.ToString()) + "WHERE PartNumber = '" + cashier.dgvCash.Rows[i].Cells[2].Value.ToString() + "'", db.GetConnection());
                        cm.ExecuteNonQuery();
                        db.closeConnection();

                        db.openConnection();
                        cm = new SqlCommand("UPDATE Cart SET Status = 'Sold' WHERE ID = '" + cashier.dgvCash.Rows[i].Cells[1].Value.ToString() + "'", db.GetConnection());
                        cm.ExecuteNonQuery();
                        db.closeConnection();
                    }
                    ...
                    MessageBox.Show("Платіж успішно збережено!", "Платіж", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    cashier.GetTranNo();
                    cashier.LoadCart();
                    this.Dispose();
                }
